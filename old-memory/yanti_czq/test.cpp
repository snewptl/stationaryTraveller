...
int main(){
	...
	init();
	for(int i=1;i<=n+1;i++){
		for(int j=1;j<=min(i-1,mxlen);j++){
			mx[j]=mx[j-1],g[j]=g[j-1];
			if(f[i-j][j]==-1)continue;
			...
			if(i==1){
				...
				if(i>j&&f[i-j][j]!=-1){
					...
					if(res==j||s[i+res]>=s[i-j+res]){
						if(dp[i][j]<dp[i-j][j]+1)dp[i][j]=dp[i-j][j]+1,f[i][j]=f[i-j][j];
						else if(dp[i][j]==dp[i-j][j]+1)f[i][j]+=f[i-j][j],mul(f[i][j]);						
					}
				}
			}
		}
