...
pll dfs(ll dep, ll p, ll sp, ll q, ll sq, ll turn, ll last) {
    ...
    if (turn == 0) {
        if (sp != 0 && sq != 0) {
            if (sp == 1) {
                return {p + p2[dep + 1] - 1, q};
            } else if (sq == 1) {
                return {p, q + p2[dep + 1] - 1};
            } else {
                return {p, q};
            }
        }
        ...
        if (sp == -1) {
            ...
            if (x & p2[dep]) {
                ...
                res2 = dfs(dep, p + p2[dep], sp, q, sq, 1, 1);
            } else {
                res1 = dfs(dep, p, sp, q, sq, 1, 0);
                res2 = dfs(dep, p + p2[dep], -1, q, sq, 1, 1);
            }
            if (check(res1) < check(res2)) return res1;
            else return res2;
        } else {
            if (sq != 0) {
                return dfs(dep, p + p2[dep], sp, q, sq, 1, 1);
            }
            pii res1, res2;
            if (x & p2[dep]) {
                res1 = dfs(dep, p, sp, q, sq, 1, 0);
        ...
        if (sq == -1) {
            ...
            if (y & p2[dep]) {
                ...
                if (!last) res2 = dfs(dep - 1, p, sp, q + p2[dep], sq, 0, 0);
            } else {
                res1 = dfs(dep - 1, p, sp, q, sq, 0, 0);
                if (!last) res2 = dfs(dep - 1, p, sp, q + p2[dep], -1, 0, 0);
            }
            if (last || check(res1) < check(res2)) return res1;
            else return res2;
        } else {
            if (!last) return dfs(dep - 1, p, sp, q + p2[dep], sq, 0, 0);
